...
也可以把任意 JSON 拖进页面，或点击“载入建筑文件”加载：
- 文件通过本地读取，不会上传到任何服务器；
- 页面内置一份示例 JSON（`data/sample.json` 的同名副本），可直接体验；
- 周边环境（地形 / 河床 / 水面 / 道路 / 护坡）同样默认已载入，直接切「周边鸟瞰」即可看到。
...
- 右键拖拽：平移
- 滚轮：缩放
- 左上角标题与浏览器标签页标题都是「生成效果图」，面板分五栏折叠（默认只展开第一栏，点标题展开/收起，
  且**同时只展开一栏**：打开新的会自动收起上一栏）：
  - **文件管理**：`载入建筑文件`（换 PlanFundingDrawing JSON，也可直接把文件拖进页面）与 `载入环境文件`（总平面图对位数据 → 地形 / 河床 / 水面 / 道路 / 护坡），另有 `选择文件…` / `清除环境`；**页面打开时两类文件都已默认载入**（示例白模 + 内嵌 site-context 副本），开箱即用
  - **模型列表**：按构件类型显示/隐藏，并在底部提示告警条数——
    - 墙体、门窗构件（框/扇/玻璃）、楼板/平台、结构柱、屋面、雨篷、楼梯/爬梯、基础/集水坑/坡道、房间标注
    - 周边地形面 / 河道河床面 / 河道水面 / 道路（DLSS 面）/ 护坡 / 环境线
  - **场景列表**：10 个固定视角一键切换 + 自定义镜头保存/删除（见下文）
  - **显示样式**：素模 / 线稿 / 深度 / 法线 四选一（见下文「出图通道」）——线稿 = 按 30° 二面角阈值提取的黑色棱边，
    **对建筑与周边环境（地形 / 河床 / 水面 / 道路 / 护坡）全部模型生效**；深度 / 法线是出图通道的实时预览
  - **渲染出图**：出图场景选择（勾选机位）+ 提示词 → `生成效果图`（条件图固定用素模截图；经本地桥调百炼，
    需先起 `tools/ai-bridge.mjs` 或双击 `start-ai-bridge.cmd`，见下文）
...
| 参数 | 作用 |
| --- | --- |
| `env=1` | 载入周边环境（地形面 / 河床面 / 水面 / 道路 / 护坡，用内嵌副本，离线可用）；**页面默认已自动载入**，加不加都一样 |
| `lines=1` | 出图含黑色线稿叠加（等价于面板「显示样式 → 线稿」，覆盖建筑与周边环境全部模型） |
| `annot=0` | 隐藏房间标注（文字 + 房间轮廓线），得到纯净条件图 |
| `ui=0` | 只留三维画面：隐藏左侧面板与底部提示条（**AI 出图条件图必加**，否则模型会把面板文字画进效果图） |
...
### 出图通道（面板「显示样式」栏 / `?channel=`）

| 显示样式 | 通道 | 说明 |
| --- | --- | --- |
| 素模 | `color` | 白模渲染，无线稿（默认） |
| 线稿 | `color` + 线稿开关 | 白模 + 黑色棱边线稿：建筑各构件与周边地形 / 河床 / 水面 / 道路 / 护坡统一按 30° 二面角阈值提棱边，挂到各 mesh 下（等价于 `?lines=1`） |
| 深度 | `depth` | 线性深度：近白远黑、纯黑背景（ControlNet Depth 友好） |
| 法线 | `normal` | 视空间法线：朝向面的 RGB ≈ (128,128,255)（ControlNet Normal 规范） |

四种样式四选一，面板里切换即时生效；depth / normal 自动隐藏线稿与标注，与素模同一相机渲染、逐像素对齐。
`WMShot.style('color'|'lines'|'depth'|'normal')` 是同一入口的脚本版。

**AI 出图的条件图**：面板固定用素模截图（`channels: ["color"]`）；需要深度 / 法线条件时走命令行
`node tools/ai-render.mjs --channels=color,depth,normal`（最多 3 张参考图，见下文）。
...
面板「渲染出图」栏：勾选要渲染的视角（12 个预设 + 自定义镜头，`看` 按钮只切机位不入队）→
改提示词（`恢复默认提示词` 从页面内嵌的 `#aiRenderData`——`data/ai-render.json` 的副本——读回）→
`生成效果图（N 次调用）`（弹窗确认后逐张发起；条件图固定素模截图）。
...
| 按钮 / 开关 | 作用 |
| --- | --- |
| **载入环境文件** | 用内嵌 `#siteContextData`（= `data/site-context.json` 副本）生成地形 / 河床 / 水面与道路 / 护坡，并切到「周边鸟瞰」视角（**页面打开时已自动载入一次**，不需要手动点） |
| **选择文件…** | 换用另一份 `site-context.json`（可用于其它项目） |
| **清除环境** | 移除环境几何、恢复建筑视角 |
| 模型列表：周边地形面 / 河道河床面 / 河道水面 / 道路（DLSS 面）/ 护坡（场地→河床）/ 环境线（岸线 / 轮廓） | 六个开关分别控制各分组；「全选 / 取消」会一并切换。DLSS 面（场地轮廓外那一圈 + 建筑周边）是**一整块实体**，开关只切换材质可见性 |
| 显示样式 → 线稿 | 地形 / 河床 / 水面 / 道路 / 护坡一起描黑边（各 mesh 挂 `EdgesGeometry` 30° 棱边），重复载入环境时旧线稿随分组一并回收（`dropDetachedOverlays()`） |

出图用法（`?env=1` 仍可用，与默认行为一致；脚本接口还能中途载入）：
