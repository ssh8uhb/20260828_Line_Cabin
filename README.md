...
当前版本：**v0.5.0**（2026-09-25，tag `v0.5.0`，用户验收后打）。v0.4.0 之后新增：
**AI 效果图链路**——白模截图 → 阿里云百炼 `qwen-image` → 效果图，两条入口共用同一调用层：
命令行 `node tools/ai-render.mjs` 与页面面板（`start-ai-bridge.cmd` 双击起本地桥，桥只绑 `127.0.0.1`，
key 只从环境变量读、不回传页面），产物落 `out/`；**左栏重构**——五栏互斥折叠
（文件管理 / 模型列表 / 场景列表 / 显示样式 / 渲染出图），标题与浏览器标签页统一为「生成效果图」，
新增「显示样式」素模 / 线稿 / 深度 / 法线四选一（线稿覆盖建筑与周边全部模型）。
v0.4.0 的**周边环境**（地形面 / 河床面 / 水面，页面打开即自动载入）与**道路 / 护坡**
（总平面图 `DLSS` / `DLSS-斜坡` 图层，整环只出一块实体）仍在（见 docs/DATA-MODEL.md 第 12、13、14 节）。
