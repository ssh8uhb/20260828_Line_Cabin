...
- **验收标准**：遍历 12 个洞口，族实例不穿墙、开向正确、尺寸与洞口一致（当前样本：门 5 樘、窗 7 扇）；
  量化判据 = `WMShot.familyCheck()` 的 `offenders = 0`，外加目视西北/东北鸟瞰无框料凸出墙面
  （坐标约定与「左手基退化」坑见 docs/DATA-MODEL.md 5.2）。
...
- **验收**：一条命令产出 12 × 通道数 张 PNG，命名含视角标识 ✅；manifest 记录相机参数 ✅。
